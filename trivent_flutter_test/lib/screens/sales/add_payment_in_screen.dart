...
class AddPaymentInScreen extends StatefulWidget {
  final PaymentInModel? existing;
  final PartyModel? prefilledParty;
  final double? prefilledAmount;
  const AddPaymentInScreen({super.key, this.existing, this.prefilledParty, this.prefilledAmount});
...
class _AddPaymentInScreenState extends State<AddPaymentInScreen> {
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final receiptNo = _isEditing
          ? widget.existing!.receiptNo
          : await _svc.nextPaymentInNo(_selectedDate);

      final model = PaymentInModel(
      ...
      if (_isEditing) {
        await _svc.updatePaymentIn(model);
      } else {
        await _svc.addPaymentIn(model);
      }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(_isEditing ? 'Edit Receipt' : 'New Payment In'),
      ),
      body: Form(
        key: _formKey,
            ...
            child: Padding(
              ...
              child: SizedBox(
                ...
                child: ElevatedButton.icon(
                  ...
                  label: Text(_saving ? 'Saving…' : (_isEditing ? 'Update' : 'Save Receipt')),
                ),
              ),
            ),
          ),
