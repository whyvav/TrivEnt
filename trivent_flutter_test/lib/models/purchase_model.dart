class PurchaseItem {
  ...
  PurchaseItem({
    required this.itemId, required this.itemName, required this.qty,
    required this.unit, required this.priceExclTax,
    this.taxPercent = 0, this.discountPercent = 0,
  });
...
class PurchaseModel {
  ...
  PurchaseModel({
    required this.id, required this.billNo, required this.partyId,
    required this.partyName, this.partyFirm, this.partyPhone,
    required this.items, required this.paymentType,
    this.paymentRef, double? amountPaid,
    DateTime? date, this.dueDate, this.notes,
  ...
  Map<String, dynamic> toMap() => {
    'id': id, 'billNo': billNo, 'partyId': partyId,
    ...
    'date': date.toIso8601String(), 'dueDate': dueDate?.toIso8601String(),
    'notes': notes,
  };

  factory PurchaseModel.fromMap(Map<String, dynamic> m) => PurchaseModel(
    id: m['id'] ?? '', billNo: m['billNo'] ?? '',
